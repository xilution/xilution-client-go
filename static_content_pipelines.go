...
func (xc *XilutionClient) CreateStaticContentPipelineEvent(organizationId *string, pipelineEvent *PipelineEvent) (*string, error) {
	...
	req, _ := retryablehttp.NewRequest("POST", fmt.Sprintf("%s/organizations/%s/pipeline-events", CoyoteBaseUrl, *organizationId), strings.NewReader(string(rb)))
...
func (xc *XilutionClient) GetStaticContentPipelineEvent(organizationId *string, pipelineEventId *string) (*PipelineEvent, error) {
	req, _ := retryablehttp.NewRequest("GET", fmt.Sprintf("%s/organizations/%s/pipeline-events/%s", CoyoteBaseUrl, *organizationId, *pipelineEventId), nil)
...
func (xc *XilutionClient) GetStaticContentPipelineEvents(organizationId *string, pageSize, pageNumber *int) (*FetchPipelineEventsResponse, error) {
	req, _ := retryablehttp.NewRequest("GET", fmt.Sprintf("%s/organizations/%s/pipeline-events?pageSize=%d&pageNumber=%d", CoyoteBaseUrl, *organizationId, *pageSize, *pageNumber), nil)
