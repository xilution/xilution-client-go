...
func (xc *XilutionClient) CreateApiPipelineEvent(organizationId *string, pipelineEvent *PipelineEvent) (*string, error) {
	...
	req, _ := retryablehttp.NewRequest("POST", fmt.Sprintf("%s/organizations/%s/pipeline-events", FoxBaseUrl, *organizationId), strings.NewReader(string(rb)))
...
func (xc *XilutionClient) GetApiPipelineEvent(organizationId *string, pipelineEventId *string) (*PipelineEvent, error) {
	req, _ := retryablehttp.NewRequest("GET", fmt.Sprintf("%s/organizations/%s/pipeline-events/%s", FoxBaseUrl, *organizationId, *pipelineEventId), nil)
...
func (xc *XilutionClient) GetApiPipelineEvents(organizationId *string, pageSize, pageNumber *int) (*FetchPipelineEventsResponse, error) {
	req, _ := retryablehttp.NewRequest("GET", fmt.Sprintf("%s/organizations/%s/pipeline-events?pageSize=%d&pageNumber=%d", FoxBaseUrl, *organizationId, *pageSize, *pageNumber), nil)
